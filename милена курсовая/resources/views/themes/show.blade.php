...
<head>
    ...
    <title>{{ $theme->title }} | Просмотр темы</title>
    <style>
        /* Анимации и эффекты */
        @keyframes fadeIn {
        ...
        .theme-actions button:hover::before {
            opacity: 1;
            left: 100%;
        }
    </style>
...
<body>
    ...
    <div class="post">
        <div>
            ...
            <div class="meta-info">
                <span>Создана: {{ $theme->created_at->format('d.m.Y H:i') }}</span>
                <span>Обновлена: {{ $theme->updated_at->format('d.m.Y H:i') }}</span>
            </div>
        </div>

        @auth
            <div class="theme-actions">
                <a href="{{ route('themes.edit', $theme->id) }}">Редактировать</a>
                <form action="{{ route('themes.destroy', $theme->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" onclick="return confirm('Вы уверены, что хотите удалить эту тему?');">Удалить</button>
                </form>
            </div>
        @endauth
    </div>
